...
uint32_t sieve(uint32_t limit, uint32_t *primes, size_t max_primes) {
    ...
    // Initialize sieve with all false (overloaded by type)
    ...
    for (int p = 3; p <= terminate; p += 2) {
        ...
        if (!sieve[idx]) {
            ...
            for (int j = ((p * p - 1) >> 1) - 1; j < sieve_length; j += p) {
                // Start at index of p^2.
                // Any prime multiple of p less than p^2 will have factors 
                // strictly less than p and will be marked seen by another prime.
                sieve[j] = true;
            }
        }
    }
